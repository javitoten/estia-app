...
function answerQuestion(companyId, question) {
  ...
  // Most / least profitable property
  ...
  // Spending on a category over N months
  ...
  return "No he entendido bien la pregunta. Puedo ayudarte con: rentabilidad por vivienda, gasto por categoría o proveedor, resumen financiero del mes, documentos y licencias por caducar, e incidencias abiertas. Prueba a reformular la pregunta con alguno de esos temas.";
}
...
module.exports = ({ post }) => {
  post("/api/ai/ask", requireApi(async (req, res) => {
    ...
    const answer = answerQuestion(companyId, question);
